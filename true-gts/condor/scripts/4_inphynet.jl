...
df = CSV.read("/mnt/dv/wid/projects4/SolisLemus-network-merging/InPhyNet-Simulations/true-gts/data/out.csv", DataFrame)
...
########## For now, some networks aren't done #################
# c = 0
for i = 1:n_subsets
    for j = 1:10
        if !isassigned(snaq_newicks, i, j)
            # c += 1
            nlls[i, j] = Inf
            runtimes[i, j] = 0.0
        end
